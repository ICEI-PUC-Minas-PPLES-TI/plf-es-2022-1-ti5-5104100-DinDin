...
class _ListCategoriesState extends State<ListCategories> {
  ...
  Future<List<Category>> fetchCategories(String type) async {
    ...
    var url = "/wallet/${widget.walletId.toString()}/category?wallet_id=" +
        widget.walletId.toString() +
        '&type=' +
        type;

    try {
      var response = await ApiURL.get(url);
      var status = response.statusCode;
      if (status == 200) {
        var json = jsonDecode(response.body);
        json['categories'].forEach((row) => {
              categoryList.add(Category(
                  id: num.parse(row['id'].toString()),
                  userId: num.parse(row['user_id'].toString()),
                  walletId: num.parse(row['wallet_id'].toString()),
