...
beforeAll(async () => {
    ...
    for (w = 1; w <= 2; w++){
        const response = await request.post("/api/wallet").send({
            description: `mockup wallet test ${w}`,
            initial_value: 0,
        });
        walletIdToSearch = response.body.wallet.id; 
        const responseINCategory = await request.post(url(`/wallet/${walletIdToSearch}/category/`)).send({
            description: "category IN",
            type: "IN",
            color: "FF0000",
        });
        INCategory = responseINCategory.body.id;
        const responseOUTCategory = await request.post(url(`/wallet/${walletIdToSearch}/category/`)).send({
        ...
        for (let i = 0; i < 5; i++) {
            ...
            const mockTransaction = {
                description: "my transaction " + w + i,
            ...
            const transactionResponse = await request    
                .post(url(`/wallet/${walletIdToSearch}/transaction`))          
    ...
    await request.put(url(`/wallet/${walletIdToSearch}/transaction`) + mockTransactionIds[1]).send({
        date: "2022-10-10",
    });
    // deletando um para filtro deleted_at
    await request.delete(url(`/wallet/${walletIdToSearch}/transaction`) + toDeleteTransaction.transaction);
});
...
describe("GET /goal test suite", () => {
    ...
    it("should list transactions filtering by wallet", async () => {
        const response = await request.get(url("/transaction?wallet_id=" + walletIdToSearch)).send();
        ...
        response.body.transactions.forEach((transaction) => {
            expect(Number(transaction.wallet_id)).toBe(walletIdToSearch);
        });
    });

    it("should list transactions filtering by description", async () => {
        const response = await request.get(url("/transaction?description=0&attribute=description")).send();
