...
beforeAll(async () => {
    ...
    for (let w = 1; w <= 2; w++) {
        ...
        const responseINCategory = await request
            .post(url(`/wallet/${walletIdToSearch}/category/`))
            .send({
        ...
        for (let i = 0; i < 5; i++) {
            ...
            if (i === 2) {
                (toDeleteTransaction.wallet = response.body.wallet.id),
                    (toDeleteTransaction.transaction =
                        transactionResponse.body.id);
            }
    ...
    await request
        .put(
            url(`/wallet/${walletIdToSearch}/transaction`) +
                mockTransactionIds[1]
        )
        .send({
            date: "2022-10-10",
        });
...
describe("GET /goal test suite", () => {
    ...
    it("should list transactions filtering by wallet", async () => {
        const response = await request
            .get(url("/transaction?wallet_id=" + walletIdToSearch))
            .send();
    ...
    it("should list transactions filtering by description", async () => {
        const response = await request
            .get(url("/transaction?description=0&attribute=description"))
            .send();
